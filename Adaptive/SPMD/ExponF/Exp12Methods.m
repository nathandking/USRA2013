function [GE, GESp]=Exp12Methods(atol,rtol,t0,tf,h0,y0,n,hx,delta,yexact)
format long;
ge=0;
geSp=0;
...
if (labindex<=4)
    ...
    while (abs(t(k)-tf) >= 1e-16)
        ...
        ztmp=EMR(:,k)+0.5*ht*fbeck(k*ht,EMR(:,k),n,hx,delta);
        ztmp(1)=0;
        ztmp(n)=0;
        EMR(:,k+1)=EMR(:,k)+ht*fbeck(k*ht,ztmp,n,hx,delta);
        EMR(1,k+1)=0;
        EMR(n,k+1)=0;
